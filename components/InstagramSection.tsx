...
export function InstagramSection() {
  ...
  return (
    <section id="instagram" className="py-16 sm:py-24 relative border-t transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full surface-card border text-[#c59b27] text-xs font-bold uppercase tracking-widest mb-3">
              <Instagram className="w-3.5 h-3.5" />
              <span>Instagram Oficial</span>
            </div>
            <h2 className="optical-title font-serif text-3xl sm:text-4xl font-black text-heading">
              Siga a Ementa Diária no Feed
            </h2>
            <p className="optical-body text-body-muted text-sm sm:text-base mt-2">
          ...
          <a
            ...
            className="apple-button inline-flex items-center gap-2 surface-card border text-heading font-bold px-5 py-3 rounded-2xl text-xs uppercase tracking-wider shadow-sm hover:border-[#c59b27]"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {posts.map((post, i) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="apple-card surface-card rounded-2xl overflow-hidden border block shadow-lg group"
            >
              <div className="aspect-square overflow-hidden relative">
                ...
                {/* Overlay */}
                <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-5 text-white">
                  <div className="flex justify-end">
                    <span className="p-2 rounded-xl bg-black/80 text-[#e5c158] border border-white/10">
